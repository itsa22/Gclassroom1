                ...
                if(in_array($img_ex_lc, $allowed_exs)){
                    ...
                    if(!empty($_POST['nom']) && !empty($_POST['prenom']) && !empty($_POST['tel']) && !empty($_POST["email"]) && !empty($_POST["mdp1"]) && !empty($_POST["mdp2"]))  {
                        if($_POST["mdp1"]==$_POST["mdp2"]){
                            ...
                            $req_recup_etu=$conn->prepare("SELECT COUNT(id_etu) AS nombre  FROM etudiant WHERE email='$email' ");
                            ...
                            if($data_etu['nombre']==1){
                                ...
                                $req_insert = $conn->prepare('INSERT INTO etudiant (id_etu, nom_etu, prenom_etu, tel, email, mot_de_passe, image_url) VALUES 
                                (NULL, :nom_etu,:prenom_etu,:tel,:email,:mdp,:image_url)');
                                ...
                                echo "<h1 style='color: green'>un compte a été crée</h1><br><a href='liste_etudiant.php'>voir la liste des etudiants</a>";
                                #$em="compte créer";
                                #header("location: create_etu.php?error=$em"); 
            ...
            if(!empty($_POST['nom']) && !empty($_POST['prenom']) && !empty($_POST['tel']) && !empty($_POST["email"]) && !empty($_POST["mdp1"]) && !empty($_POST["mdp2"]))  {
                if($_POST["mdp1"]==$_POST["mdp2"]){
                    ...
                    if($data_etu['nombre']==1){
                        ...
                        $req_insert = $conn->prepare('INSERT INTO etudiant (id_etu, nom_etu, prenom_etu, tel, email, mot_de_passe) VALUES 
                        (NULL, :nom_etu,:prenom_etu,:tel,:email,:mdp)');
...
<body>
    <center>
        <h1 style="color:white">Inscription etu</h1>
